...
export function show_warning_message(
    ...
    // The warning counterpart of show_error_message, with an optional action
    // button; like that function, it intentionally does not support HTML
    // messages.
    //
    // To prevent the same banner from appearing twice, we remove the banner
    // with a matched classname. Returns whether the banner was appended.
    $container.find(`.${CSS.escape(classname)}`).remove();
